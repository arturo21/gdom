...
| 🧩 Atributos y propiedades   | `attr`, `removeAttr`, `prop`, `data`, `addAttrb`, `rmAttrb`                           |
...
| 🧭 Traversing avanzado       | `parent`, `prev`, `next`, `siblings`, `closest`, `find`, `eq`, `not`, `filter`, `each`, `index` |
...
| 🎬 Efectos visuales          | `fadeIn`, `fadeOut`, `fadeTo`, `toggle`, `slideUp`, `slideDown`, `slideToggle`, `bounce` |
| 🌐 AJAX completo             | Integración con `ajaxapi` para `GET`, `POST`, headers, JSON, y manejo de errores      |
| 🔌 Sistema de plugins        | Extensible vía `General.JS v2` con `.extend()` o `.use()`                             |
| 🔍 Utilidades                | `version`, `getArgs`, `setCursorAtEnd`, `safeEval`, `logEvent`, `extend`              |
